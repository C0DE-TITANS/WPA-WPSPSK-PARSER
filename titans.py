   ...
   %@@@@@@@@@@@@@@@@       {G}TITANS{U}    @@@@@@*    /@@@@@@@
       #@@@@@@@@@@@@@                @@@@@@*        @@@@@@@
           (@@@@@@@@@@@@.            @@@@@@*    @@@@@@@@
               /@@@@@@@@@@@@@@@@@@@  @@@@@@*  @@@@@@
{W} Version   : {G}1.0.1{U} @@@@@@@@@@@@@@@@  @@@@@@*  @@@
{W} Github    : C0DE-TITANS
{W} Tool      : {G}WPA PSK PARSER
{G}──────────────────────────────────────────{W}""")
...
#            print("[!] Unexpected interference — kill NetworkManager/wpa_supplicant!")
    ...
    def __credentialPrint(self, wps_pin=None, wpa_psk=None, essid=None):
        ...
        print(f"[+] WPA PSK:{G} '{wpa_psk}'{W}")
        print(f"[+] AP SSID:{G} '{essid}'{W}")
...
class WiFiScanner:
    ...
    def prompt_network(self) -> str:
        ...
        while 1:
            try:
                linex()
                print(f'{W} Press Enter to Refresh Network list')
                linex()
                networkNo = input(' [•]SELECT : ')
...
def usage():
    return """
WPA PSK PARSER BY CODE-TITANS
DEVELOPED WITH : D3M09
RECODING/CHANING CREDIT not meeans you are a coder,So stay on your Limit
[-----------------------<[CODE-TITANS]>-----------------------]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='WPA WPS PSK BY CODE-TITANS (D3M09)',
    ...
    if sys.hexversion < 0x03060F0:
        die("The program requires Python 3.10 and above")
    if os.getuid() != 0:
        die("Run it as root")
